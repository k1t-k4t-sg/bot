...
- В базе данных домофона ваш ключ запишеться под иминем `(TG_username_date)`

#### Будьте внемательны при совершении данной операции, не правельно записанный ключ займет память, и может вызвать ошибку, если вы не уверены вдействии запросите информацию у ОСБ.
Данная функция выполняется многопоточно, в основном время выполнения от 30сек. 
